...
/**
 * Pull live ticket + comments for Claude.
...
export async function fetchZendeskTicketBundleForPrompt(ticketId) {
  ...
  if (!isZendeskConfigured()) {
    return {
      ...
      block: `**Zendesk Support API was not used** (no \`ZENDESK_EMAIL\` + \`ZENDESK_API_TOKEN\` — optional). Set those in \`.env.local\` for a live ticket description + comments thread; otherwise the summary relies on the PAL CSV export only.`,
    };
  }
