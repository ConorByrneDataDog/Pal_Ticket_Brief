...
import { listSupportdogSignedInDatacenters } from "@/lib/supportdogOAuthSession";
import {
...
import { findExistingTicketTldr } from "@/lib/existingTicketTldr";
...
export async function fetchSupportdogInvestigationContext(ticketId, preferredDatacenter, request) {
  ...
  const runProbe = async () => {
    ...
    for (const dc of supportdogDatacenterProbeOrder(preferred)) {
      ...
      if (attempt.ok) return { attempt, dc, errors };
      errors.push(`${dc}: ${attempt.message || attempt.code}`);
    }
    return { attempt: null, dc: null, errors };
  };

  let result = await runProbe();
  probeErrors = result.errors;

  // First attempt failed — if there's at least one signed-in region, this may be a stale
  // access token or a transient connection issue. Automatically retry once before giving up.
  if (!result.attempt && listSupportdogSignedInDatacenters(request).length > 0) {
  ...
  if (result.attempt) {
    fetched = result.attempt;
    autoDetectedDc = result.dc !== preferred;
  }
  ...
  const existingTldr = findExistingTicketTldr(parsed);
