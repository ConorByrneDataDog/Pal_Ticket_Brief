...
export default function SupportdogLandingAuth({ compact = false }) {
  ...
  /** null = follow default (collapsed once fully authenticated); true/false = user override. */
  const [expandedOverride, setExpandedOverride] = useState(/** @type {boolean | null} */ (null));
  ...
  // SupportDog access tokens are short-lived (~10 min) — poll periodically so this
  // panel doesn't show a stale "connected" state long after the session actually expired.
  useEffect(() => {
    const interval = setInterval(reload, 3 * 60 * 1000);
  ...
  if (!summary.oauthEnabled) {
    return (
      <p className="pp-glean-inline pp-section-hint">
        ...
        <code className="pp-code">.env.local</code>. Open <code className="pp-code">http://localhost:5103</code>.
      </p>
    );
  }
  ...
  const allAuthenticated = summary.checked && unsignedCount === 0;
  const expanded = expandedOverride !== null ? expandedOverride : !allAuthenticated;
    ...
    <div className={`pp-supportdog-mcp-panel${compact ? " pp-supportdog-mcp-panel--compact" : ""}`}>
      ...
      <div className="pp-supportdog-mcp-header">
        <button
          ...
          style={{ background: "none", border: "none", cursor: "pointer", padding: 0, font: "inherit", display: "inline-flex", alignItems: "center", gap: "0.3rem" }}
          onClick={() => setExpandedOverride(!expanded)}
          aria-expanded={expanded}
          title={expanded ? "Collapse" : "Expand"}
        >
          <span aria-hidden style={{ display: "inline-block", transition: "transform 0.15s", transform: expanded ? "rotate(90deg)" : "rotate(0deg)" }}>
            ›
          </span>
        ...
        <>
          ...
          {!compact && summary.checked ? (
            <p className="pp-muted" style={{ fontSize: "0.8125rem", margin: "0.35rem 0 0" }}>
              ...
              (no need to clear browser site data). Open <code className="pp-code">http://localhost:5103</code>.
            </p>
          ) : null}
        </>
      ) : null}
