...
See `pal-portfolio/.env.example` for env vars (`PAL_PORTFOLIO_CSV_PATH`, `PAL_PORTFOLIO_TICKET_MONTHS`, Snowflake MCP, Zendesk, Anthropic).
...
**Status shown in UI:** `ticketStatus` from CSV → `TICKET_STATUS` in Snowflake (`t.status`).
...
| Configuration | Path |
|---------------|------|
| `ANTHROPIC_API_KEY` set | **Claude** — investigation-playbook synthesis, `lib/claudeInvestigationPlaybook.js` (orchestrated by `lib/investigationPlaybookOrchestrator.js`) |
| No Anthropic key | **Export-only markdown** — `lib/ticketExportFallbackSummary.js` |
| Failure | **Export-only markdown** — `lib/ticketExportFallbackSummary.js` |
...
| SupportDog ticket + org context | `fetchSupportdogInvestigationContext()` | SupportDog MCP sign-in |
| docs.datadoghq.com excerpts | `fetchDatadogDocsForTicket()` | none (public) |
| Zendesk Support API | `fetchZendeskTicketBundleForPrompt()` — ticket + comments | `ZENDESK_EMAIL`, `ZENDESK_API_TOKEN`, `ZENDESK_SUBDOMAIN` |

System prompt: `lib/investigationPlaybookPrompt.js`.
...
| Briefing | CSV + Claude + SupportDog (+ optional Zendesk API) | Enrichment not from portfolio SQL |
